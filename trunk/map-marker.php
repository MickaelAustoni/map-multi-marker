...
Author URI: https://www.paypal.me/0ze
...
class MapMultiMarker {
	...
	public function mmm_plugin_install_db() {
		...
		// Load le 'upgrade.php' pour le call de la function dbDelta()
	...
	public function mmm_admin_menu() {
		add_menu_page( 'Map Multi Marker', __( 'Map Multi Marker' ), 'manage_options', 'map-multi-marker-page-manage', [
			$this,
			'mmm_rooting'
	...
	public function mmm_admin_script() {
		$localize = [
			...
			'msg_error_required' => __( 'Required fields.', self::TEXT_DOMAIN )
		];
		...
		wp_localize_script( 'mmm-admin-js', self::PREFIX . 'localize', $localize );
		wp_enqueue_media();
	}
	...
	public function mmm_front_register_script() {
		...
		wp_localize_script( 'mmm-front-js', self::PREFIX . 'localize', [ 'ajax_url' => admin_url( 'admin-ajax.php' ) ] );
	}
	...
	public function mmm_short_code( $short_code_attribute ) {
		...
		if ( $options ) {
			...
			wp_localize_script( 'mmm-front-js', self::PREFIX . 'maps', $this->maps );
