...
if( isset($_GET['create_map']) ){
	...
	//Si "$_GET['map_id']" n'est pas dans l'array "$arr_map_id" alors on crée une nouvelle row dans la base avec des options âr default
...
if( isset($_GET['edit_map']) OR isset($_GET['create_map']) ){
...
//Sinon on affiche la liste des diffrente maps déjà crée
	...
	if ( isset($_POST['securite_nonce_delete_map']) ) {
		...
		if( wp_verify_nonce($_POST['securite_nonce_delete_map'], 'securite-nonce') ) {
		    ...
		    mapmarker_alert_msg('success', __('Your map been deleted.', 'map-multi-marker'));
		}
	}
		...
		<table class="widefat" id="table_map">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<div>[map-multi-marker id="<?php echo $value['map_id']?>"]</div>
						<a href="#" class="copy-shortcode" data-id-copy="<?php echo $value['map_id']?>"><?php _e('Copy', 'map-multi-marker') ?></a>
					</td>
					<td>
						<a href="?page=map-multi-marker&edit_map&map_id=<?php echo $value['map_id']?>" class="button-primary"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> <?php _e("Edit", "map-multi-marker") ?></a>
						<button id="delete_map" name="delete_map" class="button-secondary" value="<?php echo $value['map_id']?>"><i class="fa fa-times" aria-hidden="true"></i> <?php _e("Delete", "map-multi-marker") ?></button>
					</td>
				</tr>
				<?php endforeach; ?>
			</tbody>
			<tfoot>
				<tr>
					<td><a href="?page=map-multi-marker&create_map&map_id=<?php mapMarkerGetNewMapId(); ?>" class="button-primary button-success"><i class="fa fa-map-o" aria-hidden="true"></i> <?php _e("Create new map", "map-multi-marker") ?></a></td>
					<td></td>
					<td></td>
				</tr>
			</tfoot>
