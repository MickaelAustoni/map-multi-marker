...
/*
 	//Recherche le text domaine (map-multi-marker) dans la variable $page
	$page_is_mmm = strpos($_GET['page'], $mapmarker_info['text_domaine']);
*/
...
function mapmarkerMarkerContent(){
    ...
    $option = $wpdb->get_results( "SELECT fiels_to_display, lightbox FROM " . $wpdb->prefix . 'mapmarker_option WHERE map_id="'.$_POST["shorcode_attribut"].'"', ARRAY_A);
...
function mapMarkerCreateMarker(){
	...
	//Récuère l'url des markeurs par default
    $default_marker = $wpdb->get_results( "SELECT default_marker_img_url, default_desc_img_url FROM " . $wpdb->prefix . 'mapmarker_option WHERE map_id="'.$_GET['map_id'].'"', ARRAY_A);
