...
class MapMultiMarker
{
    ...
    public function __construct()
    {
        ...
        if (is_admin()) {
            ...
            add_action('wp_enqueue_scripts', [$this, 'mmm_front_script']);
    ...
    public function mmm_admin_script()
    {
        ...
        wp_enqueue_style('fontawesome-css', $this->plugin_url . 'asset/css/font-awesome.min.css');
        wp_enqueue_style('mmm-admin-css', $this->plugin_url . 'asset/css/admin.css');
        wp_enqueue_script('mmm-admin-js', $this->plugin_url . 'asset/js/admin.js', ['jquery'], false, true);
        wp_enqueue_script('bootstrap-min-js', $this->plugin_url . 'asset/js/bootstrap.min.js', ['jquery'], false, true);
        wp_enqueue_script('clipboard-js', $this->plugin_url . 'asset/js/clipboard.min.js', ['jquery']);
     ...
     * Front script
     */
    public function mmm_front_script()
    {
        
        require_once $this->plugin_path . 'app/model/model_mapper.php';
        $model = new model_mapper();
        
        wp_enqueue_script('mmm-googlemap-js', 'https://maps.googleapis.com/maps/api/js?key=' . $model->get_api_setting()['api_key'] . '&language=fr', false, null, true);
        wp_enqueue_script('mmm-front-js', $this->plugin_url . 'asset/js/front.js', null, null, true);
        wp_enqueue_style('mmm-front-css', $this->plugin_url . 'asset/css/front.css');
        wp_enqueue_style('fontawesome-css', $this->plugin_url . 'asset/css/font-awesome.min.css');
        wp_register_style('featherlight-css', $this->plugin_url . 'asset/css/featherlight.min.css');
        wp_register_script('featherlight-js', $this->plugin_url . 'asset/js/featherlight.min.js', ['jquery']);
    ...
    {
        require_once $this->plugin_path . 'app/model/model_mapper.php';
        require_once $this->plugin_path . 'app/utility/utility.php';
        ...
        utility::mmm_prepare_array_options($options);
        utility::mmm_prepare_array_markers($markers);
        
        // Force type
        array_walk_recursive($markers_and_options, ['utility', 'mmm_force_type']);
        ...
        // Localize maps to front.js
        wp_localize_script('mmm-front-js', 'maps', $this->maps);
        
        // Optional script load
        if ($options['lightbox'] === '1') {
            wp_enqueue_style('featherlight-css', $this->plugin_url . 'asset/css/featherlight.min.css');
            wp_enqueue_script('featherlight-js', $this->plugin_url . 'asset/js/featherlight.min.js', ['jquery']);
        }
        ...
        if (shortcode_exists(self::TEXT_DOMAIN)) {
            ...
            echo '<div 
            id="map-multi-marker-' . $map_id . '" 
            class="map-multi-marker" 
            style="height: ' . $options['height_map'] . $options['height_valeur_type'] . '; width: ' . $options['width_map'] . $options['width_valeur_type'] . '"
            ></div>';
    ...
    public function mmm_async_content_marker()
    {
        ...
        if (utility::mmm_is_checked_field($option['fiels_to_display'], 'image')) {
            // if lightbox is checked
            if ($option['lightbox'] == 1) {
                ?>
                <a href="<?php echo utility::mmm_get_image_desc_src($data['img_desc_marker'], 'large') ?>"
                   data-featherlight="image">
                    <img
                            src="<?php echo utility::mmm_get_image_desc_src($data['img_desc_marker']) ?>" alt=""
                            class="img-in-marqueur <?php if (utility::mmm_is_checked_field($option['fiels_to_display'], 'image') == true AND utility::mmm_is_checked_field($option['fiels_to_display'],
                                    'titre'
                                ) == false AND utility::mmm_is_checked_field($option['fiels_to_display'], 'description') == false AND utility::mmm_is_checked_field($option['fiels_to_display'],
                                    'adresse'
                                ) == false AND utility::mmm_is_checked_field($option['fiels_to_display'], 'telephone') == false AND utility::mmm_is_checked_field($option['fiels_to_display'], 'weblink'
                                ) == false) {
                ...
                ?>
                <img src="<?php echo utility::mmm_get_image_desc_src($data['img_desc_marker']) ?>" alt=""
                     class="img-in-marqueur <?php if (utility::mmm_is_checked_field($option['fiels_to_display'], 'image') == true AND utility::mmm_is_checked_field($option['fiels_to_display'], 'titre'
                         ) == false AND utility::mmm_is_checked_field($option['fiels_to_display'], 'description') == false AND utility::mmm_is_checked_field($option['fiels_to_display'], 'adresse'
                         ) == false AND utility::mmm_is_checked_field($option['fiels_to_display'], 'telephone') == false AND utility::mmm_is_checked_field($option['fiels_to_display'], 'weblink'
                         ) == false) {
                         echo "full-width-desc";
                     } ?>">
        ...
        if (utility::mmm_is_checked_field($option['fiels_to_display'], 'titre') || utility::mmm_is_checked_field($option['fiels_to_display'], 'description'
            ) || utility::mmm_is_checked_field($option['fiels_to_display'], 'adresse') || utility::mmm_is_checked_field($option['fiels_to_display'], 'telephone'
            ) || utility::mmm_is_checked_field($option['fiels_to_display'], 'weblink')) {
            ?>

            <!-- Description des marqueurs -->
            <div class="wrap-desc-markeur <?php if (utility::mmm_is_checked_field($option['fiels_to_display'], 'image') == false) {
                echo "full-width-desc-markeur";
            } ?>">
                <!-- Si le champ du "titre" est coché -->
                <?php if (utility::mmm_is_checked_field($option['fiels_to_display'], 'titre') == true AND !empty($data['titre'])): ?>
                    <h2><?php echo html_entity_decode(esc_html($data['titre'])) ?></h2>
                <?php endif ?>

                <!-- Si le champ de la "description" est coché -->
                <?php if (utility::mmm_is_checked_field($option['fiels_to_display'], 'description') == true AND !empty($data['description'])): ?>
                ...
                <?php if (utility::mmm_is_checked_field($option['fiels_to_display'], 'adresse') == true OR utility::mmm_is_checked_field($option['fiels_to_display'], 'telephone'
                    ) == true OR utility::mmm_is_checked_field($option['fiels_to_display'], 'weblink') == true): ?>
                    <ul class="contact-list">
                        <!-- Si le champ "adresse" est coché -->
                        <?php if (utility::mmm_is_checked_field($option['fiels_to_display'], 'adresse') == true AND !empty($data['adresse'])): ?>
                            <li class="adresse">
                                <strong><?php echo html_entity_decode(esc_html($data['adresse'])) ?></strong></li>
                        <?php endif ?>
                        <!-- Si le champ de la "description" est coché -->
                        <?php if (utility::mmm_is_checked_field($option['fiels_to_display'], 'telephone') == true AND !empty($data['telephone'])): ?>
                        ...
                        <?php if (utility::mmm_is_checked_field($option['fiels_to_display'], 'weblink') == true AND !empty($data['weblink'])): ?>
