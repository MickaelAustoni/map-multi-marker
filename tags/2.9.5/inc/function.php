...
function mapmarker_admin_script() {
    ...
    wp_register_style( 'admin-css', MMM_URL.'css/admin.css');
    wp_register_script('admin-js', MMM_URL.'js/admin.js', array('jquery'), false, true);
	...
	wp_localize_script( 'admin-js', '_e', $translation_array );
...
function mapmarker_front_script($shorcode_attribut) {
	global $wpdb;

	//Get option curent map if shortcode is into page
	if(isset($shorcode_attribut['id'])){
		$option = $wpdb->get_results( "SELECT lightbox FROM " . $wpdb->prefix . 'mapmarker_option WHERE map_id="'.$shorcode_attribut['id'].'"', ARRAY_A);
	}else{
		//Init. $option
		$option = null;
	}
	...
	// Enregistre les script & css
    wp_register_style('front-css', MMM_URL.'css/front.css');
    wp_register_style('fontawesome-css', MMM_URL.'css/font-awesome.min.css');
    wp_register_script('googlemap-js', 'https://maps.googleapis.com/maps/api/js?key='.mapmarkerGetApiKey().'&language='.mapmarkerGetLanguage(), '', '', false);
...
//Load file lightBox featherlight
function featherlight_file(){
	wp_register_style('featherlight-css', MMM_URL.'css/featherlight.min.css');
	wp_register_script('featherlight-js', MMM_URL.'js/featherlight.min.js', array('jquery'));
...
function mapmarkerMarkerContent(){
	...
	if (mapmarker_get_checked_field($option[0]['fiels_to_display'], 'image') == true){
		...
		if($option[0]['lightbox'] == 1 ){
			?>
			<a href="<?php echo mapmarker_get_image_desc_src($data[0]['img_desc_marker'], 'large')?>" data-featherlight="image"><img src="<?php echo mapmarker_get_image_desc_src($data[0]['img_desc_marker'])?>" alt="" class="img-in-marqueur <?php if(mapmarker_get_checked_field($option[0]['fiels_to_display'], 'image')==true AND mapmarker_get_checked_field($option[0]['fiels_to_display'], 'titre')==false AND mapmarker_get_checked_field($option[0]['fiels_to_display'], 'description')==false AND  mapmarker_get_checked_field($option[0]['fiels_to_display'], 'adresse')==false AND mapmarker_get_checked_field($option[0]['fiels_to_display'], 'telephone')==false AND mapmarker_get_checked_field($option[0]['fiels_to_display'], 'weblink')==false){echo"full-width-desc";} ?>"></a>
			<?php
		}else{
			?>
			<img src="<?php echo mapmarker_get_image_desc_src($data[0]['img_desc_marker'])?>" alt="" class="img-in-marqueur <?php if(mapmarker_get_checked_field($option[0]['fiels_to_display'], 'image')==true AND mapmarker_get_checked_field($option[0]['fiels_to_display'], 'titre')==false AND mapmarker_get_checked_field($option[0]['fiels_to_display'], 'description')==false AND  mapmarker_get_checked_field($option[0]['fiels_to_display'], 'adresse')==false AND mapmarker_get_checked_field($option[0]['fiels_to_display'], 'telephone')==false AND mapmarker_get_checked_field($option[0]['fiels_to_display'], 'weblink')==false){echo"full-width-desc";} ?>">
			<?php
		}
    }
	...
	if (mapmarker_get_checked_field($option[0]['fiels_to_display'], 'titre')==true OR mapmarker_get_checked_field($option[0]['fiels_to_display'], 'description')==true OR  mapmarker_get_checked_field($option[0]['fiels_to_display'], 'adresse')==true OR mapmarker_get_checked_field($option[0]['fiels_to_display'], 'telephone')==true OR mapmarker_get_checked_field($option[0]['fiels_to_display'], 'weblink')==true){
	    ...
	    <div class="wrap-desc-markeur <?php if(mapmarker_get_checked_field($option[0]['fiels_to_display'], 'image')==false){echo "full-width-desc-markeur";} ?>">
	        ...
	        <?php if ( mapmarker_get_checked_field($option[0]['fiels_to_display'], 'adresse') == true OR mapmarker_get_checked_field($option[0]['fiels_to_display'], 'telephone') == true OR mapmarker_get_checked_field($option[0]['fiels_to_display'], 'weblink') == true ): ?>
	            <ul class="contact-list">
	                ...
	                <?php if (mapmarker_get_checked_field($option[0]['fiels_to_display'], 'adresse') == true AND !empty($data[0]['adresse'])): ?>
	                	<li class="adresse"><strong><?php echo html_entity_decode(esc_html($data[0]['adresse']))?></strong></li>
	                <?php endif ?>
	                <!-- Si le champ de la "description" est coché -->
	                <?php if (mapmarker_get_checked_field($option[0]['fiels_to_display'], 'telephone') == true AND !empty($data[0]['telephone'])): ?>
	                    <li class="telephone"><strong><a href="tel:<?php echo str_replace(' ','',esc_html($data[0]['telephone']))?>"><?php echo esc_html($data[0]['telephone'],2,' ')?></a></strong></li>
	                <?php endif ?>
	        ...
	        <div class="wrap-modal-child">
	            <div class="modal-confirm">
	                <form action="" method="post">
	                    ...
	                    <button id="cancel-supp" class="button-secondary"><?php _e('Cancel', 'map-multi-marker'); ?></button>
	                    <button type="submit" class="button-primary" name="submit_delete" id="submit_delete"><?php _e('Confirm', 'map-multi-marker'); ?></button>
	                </form>
	            </div>
	        </div>
	    </div>
