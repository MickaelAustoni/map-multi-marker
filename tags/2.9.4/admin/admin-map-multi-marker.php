...
if(isset($_POST['securite_nonce_upload_csv'])){
    if(wp_verify_nonce($_POST['securite_nonce_upload_csv'], 'securite_nonce_upload_csv')) {
        ...
        try{
            ...
            $wpdb->query(CsvReader::ConvertCsvToREquest($_FILES['csv_file'], $table_name, $_GET['map_id'], $options[0]));
			...
			}elseif($e->getMessage() == 'ERROR_EMPTY'){
                //Display error msg
                mapmarker_message_alert(__('Your file is empty.', 'map-multi-marker'), 'error');
			}else{
                //Display error msg
			...
			<div class="panel-heading" role="tab" id="headingTwo">
				<h4 class="panel-title">
					<a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseTwo" aria-expanded="true" aria-controls="collapseTwo" class="action-accordeon">
						<i class="fa fa-map-marker" aria-hidden="true"></i> <?php _e('Management marker', 'map-multi-marker') ?> <i class="fa fa-caret-up" aria-hidden="true"></i>
					</a>
				</h4>
			</div>
			<div id="collapseTwo" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingTwo">
				<div class="panel-body">
                    <form method="POST" action="" enctype="multipart/form-data" id="form-upload-csv">
                        ...
                        <button id="upload_csv_submit" type="submit" class="button-primary button-orange"><i class="fa fa-plus" aria-hidden="true"></i> <?php _e('Import .csv', 'map-multi-marker')?></button>
                    </form>
					    ...
					    <table class="widefat" id="table_markers">
					            ...
					            <tr>
					                ...
					                <?php
					                	if (mapmarker_get_checked_field($options[0]['fiels_to_display'], 'titre') == true) {
					                		?>
					                		<td>
					                			<input maxlength="50" type="text" value="<?php echo html_entity_decode(esc_html($item['titre']))?>" name="titre" disabled>
					                		</td>
					                		<?php
					                	}
					                ?>
					      ...
					      <tfoot>
					       <tr>
					           ...
					           <td>
					               <button type='submit' class='button-primary button-success' name="create_marker" id="create_marker"><i class="fa fa-plus" aria-hidden="true"></i> <?php _e('Add', 'map-multi-marker'); ?></button>
					           </td>
					       </tr>
					      </tfoot>
					    </table>
