...
function mapmarkerMarkerContent(){
    ...
    $option = $wpdb->get_results( "SELECT fiels_to_display FROM " . $wpdb->prefix . 'mapmarker_option WHERE map_id="'.$_POST["shorcode_attribut"].'"', ARRAY_A);
...
function mapmarker_get_image_desc_src($id){
	if(is_numeric($id) AND $id!='0' ){
		return wp_get_attachment_url($id);
	}
	else{
